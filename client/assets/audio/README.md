...
The home screen reads `tracks.json`, picks one entry at random, and moves to
another random one when it ends. Nothing else needs editing to change the music.
...
shown as the toggle's tooltip — keep `credit` accurate, it is the cheapest place
to record where a track came from.

An empty list (or a missing `tracks.json`) hides the toggle and the page behaves
exactly as before, so an install with no music is not a broken install. A single
track that 404s is dropped from the library at runtime and the rest keep playing.
...
out so entering and leaving a track is not abrupt. Playback is at volume 0.35 by
design, and visitors with `prefers-reduced-motion` never hear audio at all. The
on/off choice persists in `localStorage.aam_music`.
